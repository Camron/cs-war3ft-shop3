...
public menu_Item_Options(id){
	...
	new pos = 0, i, menu_body[512], menuitems[6][32]
	new keys = (1<<0)|(1<<1)|(1<<2)|(1<<3)|(1<<8)|(1<<9)
	...
	format(menuitems[3],31,"%L",id,"SHOW_SHOPMENU1_INFO")
	format(menuitems[4],31,"%L",id,"SHOW_SHOPMENU2_INFO")
	format(menuitems[5],31,"%L",id,"SHOW_SHOPMENU3_INFO")
...
public _menu_Item_Options(id,key){
	...
	switch (key){
		...
		case 2:	MENU_Shopmenu( id, 18 );
		case 3:	MOTD_ItemsInfo( id, 0 );
		case 4:	MOTD_ItemsInfo( id, 9 );
		case 5:	MOTD_ItemsInfo( id, 18 );
		case 8: MENU_War3Menu(id);
		default: return PLUGIN_HANDLED;
	}
...
public MENU_ReplaceItem( id )
{
	...
	pos += format( szMenu[pos], 511-pos, "\w3. %s^n", szItemName3 );
...
public MENU_Shopmenu( id, iStart )
{
	...
	//menu 3
	else if ( iStart == 2*MAX_PAGE_ITEMS )
	{
