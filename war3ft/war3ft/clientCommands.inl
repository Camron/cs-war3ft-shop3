...
CMD_Handle( id, szCmd[], bool:bThroughSay )
{
	...
	if ( get_pcvar_num( CVAR_wc3_races ) > 4 )
	{
		...
		else if ( CMD_Equal( id,  szCmd, "itemsinfo3" ) )
		{
		...
		//shop menu 3
		else if ( CMD_Equal( id, szCmd, "heprotectant" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_ORCPROT );
		}
		else if ( CMD_Equal( id, szCmd, "crittruprot" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_TRUEPROT );
		}
		else if ( CMD_Equal( id, szCmd, "stealmoney" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_STEALCASH );
		}
		else if ( CMD_Equal( id, szCmd, "stealarmor" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_STEALARMOR );
		}
		else if ( CMD_Equal( id, szCmd, "sharpbullets" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_SHARPBULLETS );
		}
		else if ( CMD_Equal( id, szCmd, "slapper" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_SLAP );
		}
		else if ( CMD_Equal( id, szCmd, "longjump" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_LONGJUMP );
		}
		else if ( CMD_Equal( id, szCmd, "mirror" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_MIRROR );
		}
		else if ( CMD_Equal( id, szCmd, "onehit" ) )
		{
			if (ITEM_MenuCanBuyCheck(id)) ITEM_Buy( id, ITEM_ONEHIT );
		}
	}
